...
let db = prodb("Productdb", {
  products: `++id, name, producer, price`,
});
...
btncreate.onclick = (event) => {
  ...
  proname.value = producer.value = price.value = "";
    ...
    if (data) {
      createEle("tr", tbody, (tr) => {
        for (const value in data) {
          createEle("td", tr, (td) => {
            td.textContent =
              data.price === data[value] ? `R$ ${data[value]}` : data[value];
          });
        }
        createEle("td", tr, (td) => {
          createEle("i", td, (i) => {
      ...
      notfound.textContent = "Nenhum registro encontrado...!";
    }
  });
}
...
const editbtn = (event) => {
  ...
  db.products.get(id, function (data) {
    ...
    producer.value = newdata.producer || "";
